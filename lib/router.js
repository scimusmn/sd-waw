Router.configure({
  /**
   * IR Common templates
   * layoutTemplate for general pages
   * notFoundTemplate for 404s
   * loadingTemplate for while we're waiting on subscriptions
   */
    ...
    waitOn: function() {
      /**
       * Put global waits here like notifications. In our app there aren't
       * any global subscriptions that we need to wait for.
       * e.g. return [Meteor.subscribe('notifications')]
       */
    }
});
...
Router.map(function() {
  ...
  this.route('componentPage', {
    ...
    onBeforeAction: function() {
        console.log('Before action, but only on component pages')
    }
...
//Router.onBeforeAction(function() { console.log 'runs reactively before the action.' });
